...
class Riskified_Full_Model_Observer{
  ...
  public function checkoutOnePageControllerSuccess(Varien_Event_Observer $observer)
  {
    if (!$observer->getEvent()) {
      Mage::log("$observer->getEvent() is empty.");
    } else {
      ...
      if (empty($orderIds) || !is_array($orderIds)) {
        ...
        foreach ($orderIds as $order_id) {
          ...
          if ($payment_details->getMethod() == 'paypal_express'){
            $this->saveOrderAfter($orderIds);
            // We are sending the full array once. So we break here.
            break;
          }
        }
      }
  ...
  public function saveOrderAfter($evt)
  {
    ...
    foreach ($order_ids as $order_id) {
      ...
      }elseif ($payment_details->getMethod() == 'paypal_express'){
        $data['shipping_lines'][]['price']	= $payment_details->getShippingAmount();
        $data['additional_info']['paypal_express_checkout_shipping_method'] = $payment_details->getAdditionalInformation('paypal_express_checkout_shipping_method');
        $data['additional_info']['paypal_payer_id'] = $payment_details->getAdditionalInformation('paypal_payer_id');
        $data['additional_info']['paypal_payer_email'] = $payment_details->getAdditionalInformation('paypal_payer_email');
        $data['additional_info']['paypal_payer_status'] = $payment_details->getAdditionalInformation('paypal_payer_status');
        $data['additional_info']['paypal_address_status'] = $payment_details->getAdditionalInformation('paypal_address_status');
        $data['additional_info']['paypal_correlation_id'] = $payment_details->getAdditionalInformation('paypal_correlation_id');
        $data['additional_info']['paypal_express_checkout_payer_id'] = $payment_details->getAdditionalInformation('paypal_express_checkout_payer_id');
        $data['additional_info']['paypal_express_checkout_token'] = $payment_details->getAdditionalInformation('paypal_express_checkout_token');
        $data['additional_info']['paypal_protection_eligibility'] = $payment_details->getAdditionalInformation('paypal_protection_eligibility');
        $data['additional_info']['paypal_payment_status'] = $payment_details->getAdditionalInformation('paypal_payment_status');
        $data['additional_info']['paypal_pending_reason'] = $payment_details->getAdditionalInformation('paypal_pending_reason');
      }else{
